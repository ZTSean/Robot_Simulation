...
RobotClass::RobotClass(double x, double y, double w, double l, double sp, double ori, ShapeType shapeType, float c[3], ConnectionType ct, int id): Shape(x, y, w, l, sp, ori, shapeType, c)
{
	...
	Wheel leftWheel(x + w/2.0 * cos(ori+ M_PI/2.0), y + w/2.0 * sin(ori+ M_PI/2.0), w, l, 80.0, ori, Circle, WheelColor, LeftWheel);
	wheels.push_back(leftWheel);
	//cout << "l wheel x and y : " << leftWheel.getXPosition() << " " << leftWheel.getYPosition()<<endl;
	//cout << "l wheel ori: " << leftWheel.getOrientation() << endl; 
	
	//-----------------------------------Right Wheel------------------------------------------------
	Wheel rightWheel(x + w/2.0 * cos(ori- M_PI/2.0), y + w/2.0 * sin(ori- M_PI/2.0), w, l, 80.0, ori, Circle, WheelColor, RightWheel);
	wheels.push_back(rightWheel);
	//cout << "r wheel x and y : " << rightWheel.getXPosition() << " " << rightWheel.getYPosition()<<endl;
	//cout << "r wheel ori: " << rightWheel.getOrientation() << endl; 
